...
const videoChoices = {
  ronaldo: { label: "Роналдо", src: defaultVideo, theme: "dark" as const },
  messi: { label: "Месси", src: "/manus-storage/asyldreams-messi_dab93a43.mp4", theme: "light" as const },
};
const featuredWorks = [
...
export default function Home() {
  ...
  const [activeVideo, setActiveVideo] = useState<keyof typeof videoChoices>("ronaldo");
  ...
  useEffect(() => { setTheme?.(videoChoices[activeVideo].theme); }, [activeVideo, setTheme]);
  ...
  function chooseVideo(video: keyof typeof videoChoices) {
    if (video === activeVideo || isVideoSwitching) return;
    setIsVideoSwitching(true);
    window.setTimeout(() => { setActiveVideo(video); setTheme?.(videoChoices[video].theme); }, 190);
    window.setTimeout(() => setIsVideoSwitching(false), 650);
  }
  ...
  return <div className={`paper-site page-transition min-h-screen overflow-x-hidden text-[#1d2547] ${isVideoSwitching ? "is-video-switching" : ""}`}>
    ...
    <main id="top">
      ...
      <section className="home-series px-4 py-16 sm:px-7 sm:py-24 lg:px-10"><div className="mx-auto max-w-[1540px]"><ScrollReveal className="mb-12 flex flex-col justify-between gap-7 md:flex-row md:items-end"><h2 className="juz-heading text-[clamp(3.2rem,6vw,6.5rem)] leading-[0.8] tracking-[-0.065em]">АВТОРСКИЕ<br />СЕРИИ</h2><Link href="/series" className="text-link">Смотреть все <ArrowDownRight className="h-4 w-4" /></Link></ScrollReveal><div className="home-series-grid">{series.map((item, index) => <ScrollReveal key={item.slug} delay={index * 70}><Link href={`/series/${item.slug}`} className="home-series-card" style={{ backgroundColor: item.color }}><img src={item.cover} alt={item.title} /><span>{item.subtitle}</span><h3 className="juz-heading">{item.title}</h3></Link></ScrollReveal>)}</div></div></section>
      ...
      <section id="about" ref={aboutSection} className="about-paper px-4 py-16 sm:px-7 sm:py-24 lg:px-10"><div className="about-paper-inner mx-auto grid max-w-[1540px] gap-10 lg:grid-cols-[.74fr_1.26fr] lg:gap-16"><div className="about-photo-wrap" onPointerMove={moveAboutPhoto} onPointerLeave={() => setPhotoTilt({ x: 0, y: 0 })} style={{ transform: `perspective(900px) rotateX(${photoTilt.y}deg) rotateY(${photoTilt.x}deg)` }}><img src="/manus-storage/asyldreams-about-character_5420337c.png" alt="Персонаж AsylDreams" className="about-photo" /></div><ScrollReveal className="about-copy"><h2 className="juz-heading text-[clamp(3.1rem,5.8vw,7.2rem)] leading-[0.8] tracking-[-0.07em]">СОБИРАЮ<br />СНЫ В КАДР</h2><p className="about-text">Я создаю AI-образы, где персонажи, цвет и атмосфера становятся отдельными историями. Pinterest AsylDreams уже смотрят тысячи людей каждый месяц.</p><div className="about-stats"><span><b>710</b> подписчиков</span><span><b>290K</b> просмотров в месяц</span><a href={pinterestUrl} target="_blank" rel="noreferrer">Pinterest <ArrowDownRight className="h-4 w-4" /></a></div></ScrollReveal></div></section>
      <section id="order" className="order-paper px-4 py-16 sm:px-7 sm:py-24 lg:px-10"><div className="order-paper-inner mx-auto max-w-[1540px] p-7 sm:p-10 lg:grid lg:grid-cols-[.8fr_1.2fr] lg:gap-16 lg:p-14"><ScrollReveal><h2 className="juz-heading text-[clamp(3.1rem,5.6vw,6.6rem)] leading-[0.78] tracking-[-0.07em]">ЗАКАЖИ<br />СВОЙ МИР</h2><p className="order-brief-note">Выберите формат, добавьте идею и референсы. Укажите дедлайн, если он важен для вашего проекта.</p></ScrollReveal><form className="order-form mt-10 lg:mt-0" onSubmit={prepareOrder}><label className="paper-field"><span>Тип AI-арта</span><select name="artType" defaultValue="Персонаж"><option>Персонаж</option><option>Портрет</option><option>Пейзаж</option><option>Обложка / постер</option><option>Серия кадров</option><option>Короткий видеофрагмент</option></select></label><label className="paper-field"><span>Где связаться</span><input name="contact" required placeholder="Instagram, Telegram или email" /></label><label className="paper-field"><span>Идея и дедлайн</span><textarea name="idea" required rows={4} placeholder="Опишите ваш мир и желаемую дату" /></label><label className="paper-field"><span>Референсы</span><textarea name="references" rows={3} placeholder="Ссылки на Pinterest, Behance или сайт" /></label><button type="submit" className="order-submit"><Send className="h-4 w-4" /> Собрать заявку</button><p className="order-expectation">Бриф можно скопировать и отправить через Pinterest или Instagram для обсуждения формата и сроков.</p>{orderStatus === "copied" && <p className="form-status"><Check className="h-4 w-4" /> Бриф скопирован</p>}{orderStatus === "ready" && <p className="form-status">Бриф готов для отправки</p>}</form></div></section>
